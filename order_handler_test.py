...
def get_current_price(session, symbol,sentiment):
    try:
        ...
        if response.get("retCode") == 0:
            ...
            ## требуется взять последнюю цену, узнать настроение рынка,
            # выделить разницу в цене последней продажи и текущей и поставить на продажу и ли покупку среднее значение
            if sentiment == "bullish":
